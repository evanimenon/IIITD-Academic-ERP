...
public class LoginPage extends JFrame {
        ...
        signIn.addActionListener(e -> {
            ...
            new Thread(() -> {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    SwingUtilities.invokeLater(() ->
                        JOptionPane.showMessageDialog(this,"Login error. Check DB and credentials.","Error",JOptionPane.ERROR_MESSAGE)
                    );
                } 
                finally {
                    SwingUtilities.invokeLater(() -> {
    ...
    // ... RoundedTextField, RoundedPasswordField, RoundedButton ...
    static class RoundedTextField extends JTextField {
        private final String placeholder;
        RoundedTextField(int cols,String placeholder){ 
            super(cols); 
    ...
    static class RoundedPasswordField extends JPasswordField {
        private final String placeholder;
        RoundedPasswordField(int cols,String placeholder){ super(cols); this.placeholder=placeholder;
    ...
    static class RoundedButton extends JButton {
        private final Color base=new Color(39,96,92), hover=new Color(28,122,120);
        RoundedButton(String text){ super(text); setFocusPainted(false); setBorderPainted(false);
            setContentAreaFilled(false); setForeground(Color.WHITE); setFont(FontKit.bold(18f));
            setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)); setBorder(new EmptyBorder(14,28,14,28));
            setRolloverEnabled(true);
        }
        @Override protected void paintComponent(Graphics g){
            boolean isHover=getModel().isRollover();
            Graphics2D g2=(Graphics2D)g.create();
            g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON);
            int arc=24, w=getWidth(), h=getHeight(), shadowOffset=isHover?4:3;
            g2.fill(new RoundRectangle2D.Double(1,shadowOffset,w-2,h-2,arc,arc));
            g2.setColor(isHover?hover:base);
            g2.fill(new RoundRectangle2D.Double(0,0,w-2,h-shadowOffset-1,arc,arc));
            g2.dispose();
